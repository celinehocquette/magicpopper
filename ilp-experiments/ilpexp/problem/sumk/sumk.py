...
def gen_list():
    return[random.randint(0, MAX) for _ in range(MAX_LIST_SIZE)]
...
def gen_pos(magic_value):
    ...
    while n2 == n1:
        n2 = random.randint(0, len(x) - 1)
    x[n2] = magic_value-x[n1]
    return f'sumk({x})'


def gen_neg(magic_value):
    x = gen_list()
    while sumk(x, magic_value):
        x = gen_list()
    return f'sumk({x})'
...
class Sumk(Problem):
    ...
    def __init__(self, num_examples=DEFAULT_NUM_EXAMPLES):
        super().__init__("sumk")
        self.magic_value = random.randint(0, MAX)
    ...
    def generate_constant_set(self, n_constants):
        constant_set = list(range(n_constants))
        return constant_set
    ...
    def generate_popper(self, system, data_path, pos_examples, neg_examples):
        return PopperTrainSettings(
            ...
            bk_file=self.mk_bk_file(data_path),
